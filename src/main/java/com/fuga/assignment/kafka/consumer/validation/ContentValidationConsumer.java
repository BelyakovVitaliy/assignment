...
@Slf4j
public class ContentValidationConsumer {

	private final ValidationResultRepository validationResultRepository;
	private final SerializationHelper serializationHelper;
	private final Clock clock;

	@KafkaListener(topics = KafkaTopics.TOPIC_VALIDATE_CONTENT, groupId = "content-validation-group")
	public void consume(String message) {
		AlbumValidationMessage event = serializationHelper.readValue(message, AlbumValidationMessage.class);
		log.info("Received content validation request for albumId={}", event.albumId());

		if (validationResultRepository.existsByAlbumIdAndType(event.albumId(), ValidationResult.ValidationType.CONTENT)) {
			log.info("Content validation already recorded for albumId={}", event.albumId());
			return;
		}

		ValidationResult result = ValidationResult.builder()
				.albumId(event.albumId())
				.type(ValidationResult.ValidationType.CONTENT)
				.createdAt(LocalDateTime.now(clock))
				.result(ValidationResult.Result.SUCCESS)
				.build();
		validationResultRepository.save(result);
		log.info("Content validation SUCCESS saved for albumId={}", event.albumId());
	}
}
